  ...
  async play(song, message) {
    ...
    try {
      if (song.url.includes("youtube.com")) {
        stream = await ytdlDiscord(song.url, { highWaterMark: 1 << 45 } , {bitrate: 192000 /* 192kbps */} , {quality: 'highestaudio'});
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔊":
          reaction.users.remove(user).catch(console.error);
          if (!canModifyQueue(member) || queue.volume == 200) return;
          if (queue.volume + 10 >= 100) queue.volume = 200;
          else queue.volume = queue.volume + 10;
          queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 200);
          queue.textChannel
